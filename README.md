...
## How to enable debug 
In order to debug the application we are going to add some extra arguments to the docker run command as seen in the previous step 'Running Locally'

Modify the docker command with the new debug arguments :
```
[root directory] $ docker run -d --name [some name] DEBUG=true -e DEBUG_PORT='*:8787' -p 8080:8080 -p '8787:8787'
...
[account id].dkr.ecr.[region].amazonaws.com/tdr-auth-server:[your build tag]
```
